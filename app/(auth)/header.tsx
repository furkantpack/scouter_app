...
function DynamicContent({ pathname }: { pathname: string }) {
  ...
  useEffect(()=>{const value=new URLSearchParams(window.location.search).get('next');if(value)setNext('?next='+encodeURIComponent(safeNext(value)));},[]);
  ...
  return (
    <>
      ...
      <Button.Root variant='primary' mode='lighter' size='xsmall' asChild>
        <Link href={linkHref+next}>{linkText}</Link>
      </Button.Root>
    </>
  );
}

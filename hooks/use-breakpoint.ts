...
const useBreakpoint = () => {
  ...
  // The server and the browser must share the same initial snapshot.
  // Actual viewport values are applied by the effect after hydration.
  const initialState = Object.keys(breakpoints).reduce(
    (acc, key) => ({
      ...acc,
      [key]: false,
    }),
